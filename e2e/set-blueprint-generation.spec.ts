...
test.describe('@smoke set blueprint generation', () => {
  test('paste blueprint JSON, apply, and recover after reload', async ({ page }) => {
    ...
    const spatiallyInvalid = structuredClone(MINIMAL_BLUEPRINT);
    spatiallyInvalid.objects[0]!.dimensions = [10, 3, 0.1];
    await page.locator('[data-set-generation-paste-input]').fill(JSON.stringify(spatiallyInvalid));
    await page.locator('[data-set-generation-review]').click();
    await expect(dialog).toContainText('dimension_axis_mismatch');
    await expect(page.locator('[data-set-generation-apply]')).toBeDisabled();
    ...
    await expect(dialog).toContainText('Hosted doorways');
    await expect(dialog).toContainText('1 / 1');
    await expect(dialog).toContainText('Centers (v2)');
