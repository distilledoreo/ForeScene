...
New imported geometry is stored as binary data in IndexedDB and referenced by a stable key in project JSON. Saving a project with model geometry creates a `.forescene-project` ZIP containing `project.json` and its binary mesh assets. Opening the package restores those assets for offline use. Legacy `.panoref-project` packages written before the rebrand use the same layout and remain importable. Existing base64-backed JSON projects still open and are migrated to binary entries when next saved. A missing binary produces a visible placeholder with a recoverable error instead of silently disappearing.
