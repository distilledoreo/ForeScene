...
- Playwright E2E (`npm run test:e2e`) auto-runs `npm run build` + `vite preview` on port 4173 unless `PLAYWRIGHT_BASE_URL` is set. Prefer tagged suites: `test:e2e:smoke`, `test:e2e:responsive`, `test:e2e:visual`, `test:e2e:heavy`, `test:e2e:webkit`. Set `PLAYWRIGHT_SKIP_BUILD=1` when `dist/` is already built. `FULL_REGRESSION=1` enables serial workers + one retry (main/nightly).
