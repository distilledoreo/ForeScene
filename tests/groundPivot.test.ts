import * as THREE from 'three';
import { describe, expect, it } from 'vitest';
import { centerTransformForBoundsPlant, centerTransformForFootPlant } from '../src/engine/groundPivot';

describe('centerTransformForFootPlant', () => {
  it('drops a center-staged root by the lean-induced foot rise', () => {
    const planted = centerTransformForFootPlant([0, 0.875, -5.3], [34, 0, 0], 1.75);
    expect(planted[0]).toBe(0);
    expect(planted[2]).toBe(-5.3);
    expect(planted[1]).toBeLessThan(0.875);
    expect(planted[1]).toBeGreaterThan(0.7);
  });

  it('leaves upright subjects at their authored origin', () => {
    expect(centerTransformForFootPlant([0, 0.875, 0], [0, 25, 0], 1.75)).toEqual([0, 0.875, 0]);
  });

  it('keeps the complete depth bound above the floor during a rigid lean', () => {
    const dimensions: [number, number, number] = [1.2, 1.4, 1.8];
    const planted = centerTransformForBoundsPlant([0, 0.7, 0], [34, 0, 0], dimensions);
    const rotation = new THREE.Euler(THREE.MathUtils.degToRad(34), 0, 0);
    const corners = [-1, 1].flatMap((x) => [-1, 1].flatMap((y) => [-1, 1].map((z) => (
      new THREE.Vector3(
        x * dimensions[0] / 2,
        y * dimensions[1] / 2,
        z * dimensions[2] / 2,
      ).applyEuler(rotation).add(new THREE.Vector3(...planted))
    ))));
    expect(Math.min(...corners.map((corner) => corner.y))).toBeCloseTo(0, 6);
  });
});
