...
type ContinuitySet = (
  partial: Partial<ContinuityStore> | ((state: ContinuityStore) => Partial<ContinuityStore>),
) => void;
type ContinuityGet = () => ContinuityStore;

/** Bound during createContinuityStoreState so post-store helpers can update state. */
...
export function createContinuityStoreState(
  ...
  landShotFraming: (shotId, camera, options) => {
    ...
    // Continuous capture (still camera) stays in fly ΓÇö don't drop pointer lock.
