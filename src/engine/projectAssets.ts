...
export function getReferencedProjectAssetIds(project: LocationProject): Set<string> {
  ...
  for (const object of project.scene.objects) {
    ...
    if (poseable?.kind === 'autorigged' || poseable?.kind === 'importedRig') {
      ...
      if (rig?.importedRigBinding?.sourceAssetId) referenced.add(rig.importedRigBinding.sourceAssetId);
  ...
  for (const shot of project.shots) {
    ...
    for (const still of Object.values(shot.materializedMedia?.stills ?? {})) {
      referenced.add(still.assetId);
    }
